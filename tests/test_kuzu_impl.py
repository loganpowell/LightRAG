...
class TestKuzuDBStorage:
    ...
    async def test_node_degree(self, storage):
        ...
        # Create nodes
        await storage.upsert_node('center_node', {
        ...
        await storage.upsert_node('connected_node_1', {
        ...
        await storage.upsert_node('connected_node_2', {
        ...
        # Add edges
        await storage.upsert_edge('center_node', 'connected_node_1', {
            'weight': 0.5,
            'description': 'edge1',
            'source_id': 'chunk_1'
        })
        
        await storage.upsert_edge('center_node', 'connected_node_2', {
            'weight': 0.7,
            'description': 'edge2',
            'source_id': 'chunk_1'
        })
    ...
    async def test_get_node_edges(self, storage):
        ...
        for i in range(3):
            await storage.upsert_node(f'connected_{i}', {
            ...
            # Create edge
            await storage.upsert_edge('main_node', f'connected_{i}', {
                'weight': 0.5 + i * 0.1,
    ...
    async def test_batch_operations(self, storage):
        ...
        for node_id in node_ids:
            await storage.upsert_node(node_id, {
                'entity_id': node_id,
                'entity_type': 'Person',
    ...
    async def test_chunk_id_queries(self, storage):
        ...
        await storage.upsert_node('chunk_node_1', {
        ...
        await storage.upsert_node('chunk_node_2', {
        ...
        await storage.upsert_edge('chunk_node_1', 'chunk_node_2', {
            'weight': 0.9,
            'description': 'chunk edge',
            'source_id': chunk_id
        })
    ...
    async def test_deletion_operations(self, storage):
        ...
        # Create test data
        await storage.upsert_node('delete_node_1', {
        ...
        await storage.upsert_node('delete_node_2', {
        ...
        await storage.upsert_edge('delete_node_1', 'delete_node_2', {
            'weight': 0.5,
    ...
    async def test_knowledge_graph_retrieval(self, storage):
        ...
        # Create edges
        await storage.upsert_edge('kg_node_1', 'kg_node_2', {
            'weight': 0.8,
            'description': 'connects to',
            'source_id': 'chunk_1'
        })
        
        await storage.upsert_edge('kg_node_2', 'kg_node_3', {
            'weight': 0.7,
    ...
    async def test_drop_operation(self, storage):
        ...
        await storage.upsert_node('drop_test_node', {
